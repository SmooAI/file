...
import { AnyWebReadableByteStreamWithFileType, FileTypeResult } from 'file-type';
...
const { setMockSteamFileTypeResult, createMockTypedStream, mockReadStream, resetMockSteamFileTypeResult, mockWriteStream } = vi.hoisted(() => {
    const mockSteamFileTypeResult: Partial<Writable<FileTypeResult>> = { mime: 'text/plain', ext: 'txt' }
    const setMockSteamFileTypeResult = (fileTypeResult: Partial<Writable<FileTypeResult>> = { mime: 'text/plain', ext: 'txt' }) => {
    ...
    const createMockTypedStream = (fileTypeOptions: Partial<FileTypeResult> = mockSteamFileTypeResult) => {
        const mockReader = {
            read: vi.fn().mockResolvedValueOnce({ done: false, value: new Uint8Array(8) })
...
vi.mock('file-type', () => {
    return {
        ...
        FileTypeParser: vi.fn().mockImplementation(() => ({
            toDetectionStream: vi.fn().mockImplementation(() => createMockTypedStream()),
        })),
    }
});
...
describe('#File', () => {
    ...
    describe('file system operations', () => {
        ...
        it('should prepend content to file', async () => {
            const file = await File.createFromFile('/example.txt', { name: 'example.txt' });
            const content = 'new content';
    ...
    describe('stream operations', () => {
        it('should pipe to writable stream', async () => {
            ...
            const mockStream = createMockTypedStream();
            vi.spyOn(file, 'getStream').mockResolvedValue(mockStream);
        ...
        it('should pipe to file', async () => {
            const file = await File.createFromBytes(new ArrayBuffer(8), { name: 'example.txt' });
            const destinationPath = './example-dest.txt';
            
            const mockStream = createMockTypedStream();
            vi.spyOn(file, 'getStream').mockResolvedValue(mockStream);
            
            await file.pipeToFile(destinationPath);
            
            expect(vi.mocked(fs.createWriteStream)(destinationPath).write).toHaveBeenCalled();
            expect(vi.mocked(fs.createWriteStream)(destinationPath).end).toHaveBeenCalled();
        });
        ...
        it('should get stream', async () => {
            ...
            expect(stream.fileType).toEqual({ ext: 'txt', mime: 'text/plain' });
        });
    });
    ...
    describe('S3 operations', () => {
        ...
        it('should upload to S3', async () => {
            ...
            expect(s3Client.send).toHaveBeenCalledWith(expect.objectContaining({
